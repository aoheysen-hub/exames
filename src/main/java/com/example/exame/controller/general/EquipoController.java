...
public class EquipoController {
    ...
    @PostMapping
    public ResponseEntity<EquipoDTO> create(@RequestBody EquipoDTO equipoDTO) throws ServiceException {
        EquipoDTO equipoDTO1 = equipoService.create(equipoDTO);
        return new ResponseEntity<>(equipoDTO1, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<EquipoDTO> update(@PathVariable Long id, @RequestBody EquipoDTO equipoDTO) throws ServiceException {
